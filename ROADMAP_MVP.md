...
| Adapter + event-driven runtime baseline | ⚠️ Partial | Trait adapters + stream events are live; `DomainEvent`/`EventBus` contracts, bounded in-process bus, runtime emitters, and tool-audit subscriber are implemented; metrics/policy subscriber migration is pending |
...
| Internal domain event bus | ⚠️ In progress (`DomainEvent` + `EventBus` contracts, bounded bus, runtime emitters, and tool-audit subscriber done; metrics/policy subscriber migration pending) |
