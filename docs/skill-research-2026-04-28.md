...
| A2 | Resource fetcher → **new `resource-finder` agent** (TOML). `skill-improver-inline` dispatches it via the orchestrator. |
...
| `tengu skill metrics <skill>` | Shipped | `src/main.rs:485` |
| `tengu skill accept-proposal` | Placeholder | `src/main.rs:509` |
| 4 `MetricKind` impls: `shell_check / llm_judge / tool_assertion / script` | Shipped | `src/adapters/skill_lifecycle/metric_kinds/` |
...
| Three-tier skill scanner (managed → workspace → project) | Shipped | `src/adapters/skill_builder.rs` + `src/adapters/rag/indexer.rs::scan_skills` |
| Auto-reindex on first chat turn picks up new skills | Shipped | `RagPlanner::auto_reindex_once` |
| Cache-discipline invariant (`loaded_in_current_conversation: false`) | Shipped | `distill.rs:204` |
| `[skill_lifecycle]` config block enabled in any sandbox | **Not enabled anywhere** | Block exists in `config.example.toml:184` but commented |
| `tengu skill remove / list / install / export / doctor` | **Missing** | — |
| Description-triggering eval | **Missing** | — |
| Human qualitative review | **Missing** | — |
| `evals/config.toml` seeded by `skill_distill` | **Missing** (known gap) | `docs/skill-lifecycle-pipeline-diagrams.md:770+` |
| Threat scanner on install | **Missing** | — |
...
|---|---|---|---|---|
| G1 | `[skill_lifecycle]` not enabled in any sandbox | Blocks `tengu skill evolve` end-to-end | D1 (TOML only) | Batch 1 |
| G2 | Phantom skill refs in `agents/researcher.toml` | Cosmetic warns | D1 | Batch 1 |
| G3 | `skill_distill` doesn't seed `evals/config.toml` | Distilled skills can't `tengu eval` | D2 | Batch 3 |
| G4 | No `tengu skill remove / list / install / export / doctor` | Operator must `rm -rf` | D2 | Batch 2 |
| G5 | No description-triggering eval | Hardest-to-debug skill failure | D2 | Batch 4 |
| G6 | No human qualitative review | Subjective skills can't be gated | D2 | Batch 5 |
| G7 | No variance in `MetricRollup` (only pass_rate) | Hides flaky metrics | D2 | Batch 6 |
| G8 | Conditional skill activation not enforced at planner-selection | Skill chosen, then fails on prereq | D2 | Batch 7 |
| G9 | No in-chat trigger phrase for `skill_distill` ("create skill from our dialog") | Tool exists but only fires by LLM discretion | D1 (SKILL.md only) | Batch 8 |
| G10 | No in-chat trigger for evaluate-current-dialog ("evaluate") | CLI-only — operator must drop to terminal mid-flow | D2 | Batch 8 |
| G11 | No reflective eval — `tengu eval` requires a pre-existing `evals/prompts.yaml`, can't use the current dialog | Forces user to pre-author fixtures | D2 | Batch 8 |
| G12 | No per-user / per-learner skill state | Two learners using `german-teacher` overwrite each other's progress | D2 (sidecar) or D3 (clones) | Batch 9 |
| G13 | No single-verb `adjust yourself` (combines evaluate + fix + resource enrichment) | User has to chain "evaluate" → "fix it" manually | D1 (planner SKILL.md) | Batch 9 |
| G14 | No resource enrichment — improver can't add YouTube links / book refs to a skill | Learning skills go stale on resources | D2 | Batch 9 |
| G15 | No teacher / learner role distinction | Anyone can edit the skill in any sandbox | D1 (frontmatter flag) | Batch 9 |
...
| # | Action | File |
|---|---|---|
| 1 | Activate `[skill_lifecycle]` + define `agents.skill-improver` + `agents.fixture-runner` | `sandboxes/aura/config.toml` |
| 2 | Drop phantom `web-research` / `summarizer` from researcher | `agents/researcher.toml` |
...
| 6 | `tengu skill doctor` | Cross-check `agents/*.toml::skills` vs filesystem. Report orphans, phantoms, missing rubric files. Exit non-zero on phantoms. Run scanner over installed skills (informational). |
...
| 22 | Plan shape: `{kind: "plan", steps: [{agent: "skill-author", goal: "distill skill from messages [N..current]"}]}`. New agent `skill-author` (`agents/skill-author.toml`) gets `workspace_tools = ["skill_distill"]` + identity instructions to call `skill_distill` with `from_message_index = N`. Already-built tool, new agent wrapper. |
...
| 24 | New agent `skill-evaluator` (`agents/skill-evaluator.toml`): given a skill name + a message-index range, runs the existing `eval_builder::run_skill` against an ephemeral `prompts.yaml` derived from the dialog slice, returns a structured pass/fail summary. |
...
| 28 | **`resource-finder` agent** (per A2). New `agents/resource-finder.toml` with `workspace_tools = ["http_request"]`. Identity: "Given a topic + learner gap description, find 2–5 high-quality web resources, return JSON `[{url, title, why_relevant, length}]`". | `agents/resource-finder.toml` |
...
| `tengu skill doctor` | `detects_phantom_refs_in_agents_toml`, `detects_orphan_skills`, `detects_missing_rubric_files`, `reports_script_metrics_informational` |
...
```
SKL-1  list shows all 8 in-tree skills, marks Script/ShellCheck
SKL-2  remove eth-balance-check cleans project-tier; managed/workspace untouched without --tier
SKL-3  doctor flags researcher.toml's phantom skills (exit non-zero); reports script metrics (informational)
...
| Cache discipline | New skills don't hot-load. `skill_distill` returns `loaded_in_current_conversation: false`. Auto-reindex on next chat boot. |
...
| Metric kinds | `src/adapters/skill_lifecycle/metric_kinds/{shell_check,llm_judge,tool_assertion,script}.rs` |
