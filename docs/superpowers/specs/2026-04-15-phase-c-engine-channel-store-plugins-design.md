...
**Date:** 2026-04-15
**Depends on:** Phase B (tool plugin architecture) and Phase A (orchestration collapse) — both must be complete.
...
After Phase B, tool extensibility is clean: adding a new tool plugin or a new MCP server is a one-file change. After Phase A, orchestration behaviour lives in skills rather than Rust.
...
- **Channels** — `CliChannel` (in `chat_builder.rs`) and `TelegramChannel` (in `telegram_builder.rs`, feature-gated on `telegram`). After Phase A both drop through `channel_runtime.rs`, but the construction sites are still hard-coded. Adding Slack, Discord, Matrix, or mail today means editing several files.
...
Each extension point has its own ad-hoc pattern. None of them use the plugin + registry pattern Phase B established for tools, even though they would fit it exactly.

## 2. Goal

Apply the Phase B plugin pattern uniformly to engines, channels, memory stores, and embedders. Feature flags collapse to a single `#[cfg]` per `match` arm in each registry — the same place the tool plugins live. Adding a new backend becomes a one-file, one-config-entry change, identical in shape to adding a tool plugin.
...
Four new plugin traits, each mirroring `ToolPlugin` from Phase B:
...
pub(crate) struct ChannelCtx<'a> {
    ...
    pub tools:    &'a ToolRegistry,     // from Phase B
    pub plugins:  &'a PluginRegistry,   // from Phase B
...
- `TelegramChannel` — wraps the post-A3 Telegram adapter.
...
**Phase A §11.1 is the explicit contract that makes this possible.** Telegram and CLI must already share the same `open_session → send_user_message → stream_output` flow before Phase C can factor them into `ChannelPlugin` trivially.
...
`MemoryStorePort` is the existing trait in `ports.rs` (modernized to `#[async_trait]` in Phase B §4.7).
...
pub(crate) struct PluginRegistry {
    pub tools:    ToolRegistry,            // from Phase B
...
To be detailed in the full Phase C spec after Phase B + A land. Rough sketch:

| # | Step | Risk |
|---|---|---|
| C1 | `EnginePlugin` trait + `EngineRegistry` + `OpenRouterEnginePlugin` + `ClaudeCodeEnginePlugin`. Wire into boot. Feature flags move. | med |
| C2 | `ChannelPlugin` trait + `ChannelRegistry` + `CliChannel` + `TelegramChannel` (extraction from post-A3 code). | low |
...
- **Scope boundary with Phase B's `McpPlugin`.** Both `McpPlugin` (Phase B) and `ChannelPlugin` (Phase C) can expose Tengu to external clients in different ways. `McpPlugin` lets Tengu *call* external tools; Phase B's rewritten `mcp_bridge.rs` lets external MCP clients *call* Tengu tools; `ChannelPlugin` is how users interact with Tengu agents via messengers. Three different directions — keep them distinct in docs.

## 7. What's intentionally left for the full spec

- Exact trait signatures after we've built Phase B and learned what the `ToolCtx` pattern is actually ergonomic for.
- LOC numbers per PR.
...
**Do not start Phase C until Phase B and Phase A are both merged and exercised for at least a week.** The plugin pattern may need small adjustments based on what Phase B surfaces; doing C too early means rewriting it. The stub exists only so the trajectory is visible and Phase A's Telegram migration can be shaped deliberately to unblock C.
