...
fn build_eval_config_toml(
    ...
    let (engine, model) =
        engine_model.unwrap_or(("openrouter", "anthropic/claude-sonnet-4-6"));
...
mod tests {
    ...
    async fn seeds_eval_config_workspace_tools_from_fixture_tool_calls() {
        ...
        let eval_config = std::fs::read_to_string(
            ws.path()
                .join("skills/fetch-and-store/evals/config.toml"),
        )
        .unwrap();
    ...
    async fn seeds_eval_config_engine_and_model_from_calling_agent() {
        ...
        let eval_config = std::fs::read_to_string(
            ws.path().join("skills/opus-distilled/evals/config.toml"),
        )
        .unwrap();
        assert!(
