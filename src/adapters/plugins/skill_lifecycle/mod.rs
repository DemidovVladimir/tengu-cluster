...
impl ToolPlugin for SkillLifecyclePlugin {
    ...
    async fn tools(&self, _ctx: &PluginCtx<'_>) -> Result<Vec<Arc<dyn Tool>>> {
        ...
        // `register_core_plugins` decides which the agent actually sees.
        Ok(vec![
...
/// Both lifecycle tool defs concatenated — used by compute_bridge_tools when
/// either is in the workspace_tools list.
pub(crate) fn tool_defs() -> Vec<ToolDef> {
    let mut v = distill_tool_defs();
    v.extend(apply_improver_tool_defs());
    v
}
